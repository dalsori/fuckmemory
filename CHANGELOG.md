...
- **Bus / inbox: active messaging between agents.** `fuckmemory bus send "msg" [--to <agent>] [--channel <name>] [--ttl <sec>]` + `bus poll|list` + `inbox` alias, plus MCP tools `bus_send`/`bus_poll`. Messages are scoped per project, stored in `bus_messages` with `bus_cursors` per agent, auto-injected on the next prompt (400 token budget, ordered inbox → recall). Broadcast or directed, TTL expiry, channel filtering. No server, just SQLite.
- **Heartbeats: live agent status.** Every prompt touches `heartbeats(scope_id, agent, last_at)`. `session list` now shows `● working / ○ idle` per agent (3 min window) alongside sessions — the herdr-style sidebar without a server. Schema v4 adds `bus_*` + `heartbeats` tables.
- **Plugin skeleton (herdr-style).** `~/.local/share/fuckmemory/plugins/<name>/plugin.toml` (`[plugin] name/version/command/hooks = ["on_store"]`) + `plugin.json` support. `fuckmemory plugin list|install <src>|uninstall <name>` discovers them, and `hook prompt` runs `on_store` plugins as a subprocess (stdin JSON → stdout JSON, 200 ms timeout, never breaks the hook). Foundation for `on_recall` and a marketplace in 1.5.0.
