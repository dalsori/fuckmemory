...
fn bus_ttl_expiry_via_cli() {
    ...
    assert!(out.contains("empty") || !out.contains("ephemeral"), "ttl not expired: {out}");
}
...
fn bus_broadcast_vs_directed_via_cli() {
    ...
    assert!(out.contains("broadcast_msg"), "opencode missing broadcast: {out}");
    assert!(out.contains("private_msg"), "opencode missing private: {out}");
    ...
    assert!(out3.contains("broadcast_msg"), "codex missing broadcast: {out3}");
    assert!(!out3.contains("private_msg"), "codex should not see private: {out3}");
}
...
fn heartbeat_shows_in_session_list() {
    ...
    hook(&home, &["hook", "prompt", "--agent", "claude-code"], &p1, &env);
    ...
    assert!(ok);
    assert!(out.contains("claude-code") || out.contains("opencode"), "session list missing agents: {out}");
    // heartbeat footer should show working
    assert!(out.contains("working") || out.contains("agents"), "heartbeat not shown: {out}");
}
...
fn hook_inbox_injection_only_for_worthwhile_prompts() {
    ...
    // send a bus message
    run(&home, &["bus", "send", "important: check deploy", "--to", "opencode"]);
    // ack prompt "ok" should NOT get inbox (worth_recalling false)
    let ack = payload(&proj, "ok", "sid-ack");
    let (out, _, ok) = hook(&home, &["hook", "prompt", "--agent", "opencode"], &ack, &autosave);
    assert!(ok);
    ...
    assert!(!out.contains("Inbox") || out.is_empty(), "ack should not inject inbox: {out}");
    // now a real prompt should get inbox
    let real = payload(&proj, "implement the feature", "sid-real");
    let (out2, _, ok2) = hook(&home, &["hook", "prompt", "--agent", "opencode"], &real, &autosave);
    assert!(ok2);
    assert!(out2.contains("Inbox") && out2.contains("check deploy"), "real prompt should inject inbox: {out2}");
    // second real prompt should have empty inbox (consumed)
    let real2 = payload(&proj, "continue implementation", "sid-real");
    let (out3, _, _) = hook(&home, &["hook", "prompt", "--agent", "opencode"], &real2, &autosave);
    assert!(!out3.contains("check deploy"), "inbox should be consumed: {out3}");
}
...
fn plugin_list_shows_invalid_and_valid() {
    ...
    assert!(out.contains("bad-plug") || out.contains("invalid"), "bad not shown: {out}");
}
...
fn mcp_bus_round_trip() {
    ...
    // initialize
    send(serde_json::json!({"jsonrpc":"2.0","id":1,"method":"initialize","params":{"protocolVersion":"2025-06-18","capabilities":{}}}));
    let _ = recv(&mut reader);
    send(serde_json::json!({"jsonrpc":"2.0","method":"notifications/initialized"}));
    // bus_send
    send(serde_json::json!({"jsonrpc":"2.0","id":2,"method":"tools/call","params":{"name":"bus_send","arguments":{"body":"hello via mcp","channel":"test"}}}));
    let r = recv(&mut reader);
    assert!(r["result"]["content"][0]["text"].as_str().unwrap().contains("sent"), "bus_send failed: {r}");
    ...
    send(serde_json::json!({"jsonrpc":"2.0","id":3,"method":"tools/call","params":{"name":"bus_poll","arguments":{}}}));
    ...
    assert!(txt.contains("hello via mcp") || txt.contains("Inbox") || txt.contains("empty") == false || txt.contains("mcp"), "bus_poll unexpected: {txt} {r2}");
    drop(stdin);
    let _ = child.wait();
}
